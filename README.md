...
WorkVitor = game:GetService("WorkVitor")
...
local ActiveFolder = WorkVitor:FindFirstChild("active")
                ...
                if tool then
                    ...
                    if hasBobber then
                        ...
                        if selectedZoneCast == "Bluefin Tuna Abundance" then
                            local selectedZone = WorkVitor.zones.fishing:FindFirstChild("Deep Ocean")
                        ...
                        elseif selectedZoneCast == "Swordfish Abundance" then
                            ...
                            -- Default Zones
                            local selectedZone = WorkVitor.zones.fishing:FindFirstChild(selectedZoneCast)
                            if selectedZone then
...
function Appraise()
    spawn(function()
        while AutoAppraiser do
            workVitor:WaitForChild("world"):WaitForChild("npcs"):WaitForChild("Appraiser"):WaitForChild("appraiser"):WaitForChild("appraise"):InvokeServer()
            task.wait(0.1)
        end
    end)
end
...
function SellFishAndReturnAll()
    ...
    task.wait(0.5)
    workVitor:WaitForChild("world"):WaitForChild("npcs"):WaitForChild("Marc Merchant"):WaitForChild("merchant"):WaitForChild("sellall"):InvokeServer()
    task.wait(3)
...
function SellFishAndReturnOne()
    ...
    rootPart.CFrame = sellPosition
    task.wait(0.5)
    workVitor:WaitForChild("world"):WaitForChild("npcs"):WaitForChild("Marc Merchant"):WaitForChild("merchant"):WaitForChild("sell"):InvokeServer()
    task.wait(3)
...
do
    ...
    DropdownH:OnChanged(function(Value)
        ...
        if HumanoidRootPart ~= nil and ActiveFolder ~= nil then
            ...
            if EventItem ~= nil and EventItem:FindFirstChild("PickupPrompt") ~= nil then
                ...
                Fluent:Notify({
                    Title = "WARNING",
                    Content = string.format('There is no "%s" in workVitor', Value),
                    Duration = 8
                })                
            end
        end
    ...
    Tabs.Misc:AddButton({
        Title = "Delete flags",
        Callback = function()
            local flags = workVitor.active.flags
            if flags then
                flags:Destroy()
            end
        end
...
InterfaceManager:SetFolder("VitorHub")
SaveManager:SetFolder("VitorHub/Fisch")
...
if deviceType == "Mobile" then
    ...
    B.MouseButton1Click:Connect(function()
        local VitorGui = game.CoreGui:FindFirstChild("ScreenGui") -- Replace with the actual name
        if VitorGui then
            toggleVisibility(VitorGui)
        else
            warn("1")
        end
    end)
